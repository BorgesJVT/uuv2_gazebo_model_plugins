...
#include "uuv2_gazebo_model_plugins/ThrusterPlugin.hpp"
#include "uuv2_gazebo_model_plugins/Def.hpp"


GZ_REGISTER_MODEL_PLUGIN(uuv2_gazebo_model_plugins::ThrusterPlugin)
  ...
  sdf::ElementPtr _sdf)
{
  ...
  if (this->thrustMin >= this->thrustMax) {
    gzmsg << "thrustMax must be greater than thrustMin, returning to default values..." <<
      std::endl;
  ...
  this->thrustTopicPublisher =
    this->node->Advertise<gazebo::msgs::Vector3d>(this->topicPrefix + "thrust");

  // Subscribe to the input signal topic

  this->commandSubscriber =
    this->node->Subscribe(
    this->topicPrefix + "input",
  ...
  this->updateConnection = gazebo::event::Events::ConnectWorldUpdateBegin(
    boost::bind(
      &ThrusterPlugin::Update,
      this, _1));
#if GAZEBO_MAJOR_VERSION >= 8
  this->thrusterAxis =
    this->joint->WorldPose().Rot().RotateVectorReverse(this->joint->GlobalAxis(0));
#else
  this->thrusterAxis = this->joint->GetWorldPose().rot.Ign().RotateVectorReverse(
    this->joint->GetGlobalAxis(
      0).Ign());
#endif
}
...
/////////////////////////////////////////////////
void ThrusterPlugin::Update(const gazebo::common::UpdateInfo & _info)
{
  GZ_ASSERT(
    !std::isnan(this->inputCommand),
